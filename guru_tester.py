...
def remove_spread(df): #this function sets the ask and bid prices equal to their mid price equivalent hence removing the spread
    for a in ["ask", "bid"]:
        for b in ["o", "h", "l", "c"]:
            c = f"{a}_{b}"
            df[c] = df[f"mid_{b}"]

def apply_signals(df, PROFIT_FACTOR, sig): # sig here is a function
...
def create_signals(df, time_d=1): #time_d here is set to one hour but if you are gonna merge 4hr candles with 5m for example this would need to be 4 hours
